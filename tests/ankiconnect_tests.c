...
Ensure(AnkiConnectC, sends_correct_guiBrowse_request) {
    ...
    s8 expected = S("{ \"action\": \"guiBrowse\", \"version\": 6, \"params\": { \"query\" : \"\\\"deck:Japanese\\\" \\\"VocabKanji:面白い\\\"\" } }");
...
Ensure(AnkiConnectC, sends_correct_search_request) {
    ...
    s8 expected = S("{ \"action\": \"findCards\", \"version\": 6, \"params\": { \"query\" : \"\\\"deck:Japanese\\\" \\\"VocabKanji:敷衍\\\" -is:suspended\" } }");
    assert_that(received.s, is_equal_to_string((const char *)expected.s));
}
