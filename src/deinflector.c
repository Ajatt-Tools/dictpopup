...
/* 	(str.len >= lengthof(prefix) && !u8compare(str.s, (u8*)prefix,
 * lengthof(prefix))) */

/* #define endswith(str, suffix) \ */
/* 	(str.len >= lengthof(suffix) && !u8compare(str.s + str.len -
 * lengthof(suffix), (u8*)suffix, lengthof(suffix))) */
...
// can be used 	 Or just use multiple with different arguement length
#define IF_ENDSWITH_REPLACE(ending, ...)                                                           \
    if (endswith(word, S(ending))) {                                                               \
        for (const char **iterator = (const char *[]){__VA_ARGS__, NULL}; *iterator; iterator++) { \
...
void kata2hira(s8 kata_in) {
    ...
    while (*h) {
        ...
        if (h[0] == 0xe3 && (h[1] == 0x82 || h[1] == 0x83) && h[2] != '\0') {
            ...
            if ((h[1] == 0x82 && h[2] >= 0xa1) || (h[1] == 0x83 && h[2] <= 0xb6) ||
 ...
 */
static void atou_form(s8 word, ptrdiff_t len_ending) {
    word.len -= len_ending;
    assert(word.len >= 0);
 ...
 */
static void itou_form(s8 word, ptrdiff_t len_ending) {
    word.len -= len_ending;
    assert(word.len >= 0);
...
static void check_adjective(s8 word) {
