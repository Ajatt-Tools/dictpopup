...
dictentry dm_increment(DictManager self[static 1]) {
    self->index_visible =
        (self->index_visible < dictLen(self->dictionary) - 1) ? self->index_visible + 1 : 0;
    return dm_get_currently_visible(*self);
}

dictentry dm_decrement(DictManager self[static 1]) {
    self->index_visible =
        (self->index_visible > 0) ? self->index_visible - 1 : dictLen(self->dictionary) - 1;
    return dm_get_currently_visible(*self);
}
...
size_t dm_get_total_num_of_entries(const DictManager self) {
    return dictLen(self.dictionary);
}
