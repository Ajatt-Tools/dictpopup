...
static void
add_filename(database db, s8 headw, s8 fullpth)
{
    addtodb1(db, headw, fullpth);
}

static void
add_fileinfo(database db, s8 fullpth, fileinfo fi)
{
    s8 sep = S("\0");
...
static void
add_from_index(database db, char* index_path, s8 curdir)
{
...
static void
jppron_create(char* audio_dir_path, s8 dbpth)
{
...
static void
play_word(s8 word, s8 reading, s8 database_path)
{
    ...
    debug_msg("Playing word: %.*s with reading: %.*s",
	      (int)word.len, (char*)word.s,
	      (int)normread.len, (char*)normread.s);
    if (reading.len)
    {
	bool match = false;
 ...
 */
void
jppron(s8 word, s8 reading, char* audiopth)
{
